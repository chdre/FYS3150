...
In this paper we will show how to transform problems such that it can be solved as an eigenvalue problem by Jacobi's method. We will start by considering a beam fastened in both ends while a force F acts on it, and further apply the method to a quantum mechanical system of an electron in a harmonic oscillator potential. Lastly we will be extending the method to a two electron system in a harmonic oscillator potential that interacts via Coulomb repulsion. By scaling the differential equation that describes the systems above, we obtain an equation that can be solved as an eigenvalue problem. The equation found is then discretized so that we transform the equation to matrix form, as discussed in our previous paper\cite{1}, which then allows us to apply the problem to Jacobi's method. 


\section{Method}
\label{sec:method}
The method presented will show how to solve an eigenvalue problem using Jacobi's method and that by scaling the equations for a given problem, one can transform the problem to an eigenvalue problem on a matrix form with known analytical eigenpairs. The first problem is a classical one involving a beam fastened in both ends. Further the method is transformed to a quantum mechanical three dimensional quantum dot problem, starting with one electron and moving over to a problem with two electrons.
...
\tablecomments{Table comparing CPU-time between Armadillo's built in function \textbf{eig_sym} and Jacobi's method, as shown from these data Jacobi's method increases in CPU time faster than that of Armadillo's function.}
\tablecolumns{4}
