...
if data == "pos3g":
    ...
    x3 = np.loadtxt("data/3g.txt", usecols=4)
    y3 = np.loadtxt("data/3g.txt", usecols=5)
    ...
    plt.plot(x2,y2)
    plt.plot(x3,y3)
    plt.title("Solar system of Earth, Mercury and Sun",size=15)
    plt.xlabel("x",size=15); plt.ylabel("y",size=15)
    plt.legend(['Earth','Mercury', 'Sun'],prop={'size': 15},loc='upper right')
    plt.show()
